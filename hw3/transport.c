...
/* -- Receive buffer -- */
static void insert_before_node (segment_list_t *list, segment_t *target,
                                tcp_seq seq, const uint8_t *data,
                                size_t payload_len, bool_t is_fin);
static void insert_tail (segment_list_t *list, tcp_seq seq,
                         const uint8_t *data, size_t payload_len,
                         bool_t is_fin);
static void
insert_segment_to_receive_buffer_robust (segment_list_t *receive_buffer,
                                         tcp_seq seq, const uint8_t *data,
                                         size_t payload_len, bool_t is_fin);
...
// for debug
static void
print_receive_buffer (segment_list_t *receive_buffer)
{
    ...
    while (cur)
    {
        tcp_seq start = cur->seq;
        tcp_seq end = cur->seq + cur->payload_len + (cur->is_FIN ? 1 : 0);
        printf ("  [%d] seq=%u, end=%u, len=%zu, FIN=%d\n", i, start, end,
                cur->payload_len, cur->is_FIN);
        cur = cur->next;
        ++i;
    }
    printf ("========================\n");
...
static void
insert_before_node (segment_list_t *list, segment_t *target, tcp_seq seq,
                    const uint8_t *data, size_t payload_len, bool_t is_fin)
{
    ...
    // If target node is head, then inserted node becomes new head
    if (list->head == target)
        list->head = node;
    else
    {
        // else find the node before the target node
        segment_t *prev = list->head;
...
static void
insert_tail (segment_list_t *list, tcp_seq seq, const uint8_t *data,
             size_t payload_len, bool_t is_fin)
{
    segment_t *node = malloc (sizeof (segment_t));
    node->seq = seq;
...
/* Assuming that segment can be sent with overlapped byte */
static void
insert_segment_to_receive_buffer_robust (segment_list_t *receive_buffer,
                                         tcp_seq seq, const uint8_t *data,
                                         size_t payload_len, bool_t is_fin)
{
    /*
     * Robust segment insertion algorithm for receive buffer:
     *
     * 1. Initialize: seq_start = seq, seq_end = seq_start + payload_len +
     * is_FIN, pos = seq_start.
     * 2. Traverse existing nodes: For each node, let node_start = cur->seq,
     * node_end = cur->seq + cur->payload_len + cur->is_FIN.
     *    - While (cur exists && pos < seq_end):
     *      a. If (pos < node_start): // There is a gap before this node
     *         - Insert [pos, min(node_start, seq_end)).
     *         - Update pos = min(node_start, seq_end).
     *         - If (pos >= seq_end) break.
     *      b. If (pos < node_end): // Overlapping part with this node
     *         - Skip overlapped region by pos = min(node_end, seq_end).
     *         - If (pos >= seq_end) break.
     *      c. Otherwise, move to next node.
     * 3. After loop, if (pos < seq_end), insert remaining [pos, seq_end) at
     * the end.
     *
     * The algorithm ensures that only non-overlapping, not-yet-received parts
     * are inserted, maintaining a sorted, non-overlapping linked list of
     * segments in the receive buffer.
     */

    assert (payload_len > 0 || is_fin);

    tcp_seq seg_start = seq;
    tcp_seq seg_end = seg_start + payload_len + (is_fin ? 1 : 0);
    tcp_seq pos = seg_start;

    segment_t *cur = receive_buffer->head;

    while (cur && pos < seg_end)
    {
        tcp_seq node_start = cur->seq;
        tcp_seq node_end = cur->seq + cur->payload_len + (cur->is_FIN ? 1 : 0);

        if (pos < node_start)
        {
            tcp_seq insert_start = pos;
            tcp_seq insert_end = node_start < seg_end ? node_start : seg_end;
            size_t seq_gap = insert_end - insert_start;
            bool_t fin_here = (insert_end == seg_end) && is_fin;

            insert_before_node (receive_buffer, cur, insert_start,
                                data + (insert_start - seg_start),
                                seq_gap - fin_here, fin_here);
            pos = insert_end;
            if (pos >= seg_end)
                return;
        }
        ...
        if (pos < node_end)
            pos = node_end < seg_end ? node_end : seg_end;
    ...
    {
        tcp_seq insert_start = pos;
        tcp_seq insert_end = seg_end;
        size_t chunk = insert_end - insert_start;
        bool_t fin_here = (insert_end == seg_end) && is_fin;

        insert_tail (receive_buffer, insert_start,
                     data + (insert_start - seg_start),
                     chunk - (fin_here ? 1 : 0), fin_here);
    }
}
...
handle_app_data (mysocket_t sd, context_t *ctx)
{
    ...
    /* Below comment is for testing receive side robust buffering */
    /* size_t q1 = payload_len / 4;
    size_t q2 = payload_len / 2;
    size_t q3 = (payload_len * 3) / 4;

    printf ("payload_len: %zu\n", payload_len);
    printf (
        "sent packet range: [%zu, %zu), [%zu, %zu), [%zu, %zu), [%zu, %zu)\n",
        ctx->next_seq_num, ctx->next_seq_num + q1, ctx->next_seq_num + q1,
        ctx->next_seq_num + q2, ctx->next_seq_num + q2,
        ctx->next_seq_num + payload_len, ctx->next_seq_num + q3,
    ...
    if (q2 > 0)
    {
        STCPHeader hdr;
        build_header (&hdr, ctx->next_seq_num, ctx->receive_next, TH_ACK);
        stcp_network_send (sd, &hdr, sizeof (STCPHeader), payload, q2, NULL);
    }

    if (q3 > q1)
    {
        STCPHeader hdr;
        build_header (&hdr, ctx->next_seq_num + q1, ctx->receive_next, TH_ACK);
        stcp_network_send (sd, &hdr, sizeof (STCPHeader), payload + q1,
                           q3 - q1, NULL);
    }
    ...
    {
        STCPHeader hdr;
        build_header (&hdr, ctx->next_seq_num + q2, ctx->receive_next, TH_ACK);
        stcp_network_send (sd, &hdr, sizeof (STCPHeader), payload + q2,
                           payload_len - q2, NULL);
    }
    ...
    {
        STCPHeader hdr;
        build_header (&hdr, ctx->next_seq_num + q3, ctx->receive_next, TH_ACK);
        stcp_network_send (sd, &hdr, sizeof (STCPHeader), payload + q3,
                           payload_len - q3, NULL);
    } */
                         ...
                         bool_t is_fin)
{
    ...
    if (seq_start < win_start)
    {
        size_t cut = win_start - seq_start;
        seq_start += cut;
        if (cut < payload_len)
        {
            payload += cut;
    ...
    insert_segment_to_receive_buffer_robust (&ctx->receive_buffer, seq_start,
                                             payload, payload_len, is_fin);
