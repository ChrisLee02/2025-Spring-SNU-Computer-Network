...
volatile sig_atomic_t active_connections = 0;

void handle_signal(int signo) {
  fprintf(stderr, "Received terminating signal\n");
  g_stop = 1;
}
...
void handle_new_connection(int listen_fd, int epoll_fd) {
  ...
  // ** if g_stop is set, accept() should be called to remove the socket
  if (g_stop) {
...
void handle_epollin(connection_context* ctx, int epoll_fd) {
  while (1) {
    ...
    if (strstr(ctx->read_buffer, CRLFCRLF) != NULL) {
      ...
      // ** graceful termination
      // ** if g_stop is set, make response with Connection: close
      if (g_stop) {
        ctx->request.keep_alive = CLOSE;
      }
...
void handle_epollout(connection_context* ctx, int epoll_fd) {
  ...
  if (ctx->write_phase == RESP_WRITING_BODY) {
    ...
    // ** graceful termination
    // ** if g_stop is set or connection is not keep-alive,
    // ** close the connection
    if (g_stop || ctx->request.keep_alive == CLOSE) {
      close_connection(ctx, epoll_fd);
    } else {
      reset_context(ctx);
      struct epoll_event ev = {.events = EPOLLIN | EPOLLET, .data.ptr = ctx};
...
int main(const int argc, const char** argv) {
  ...
  while (1) {
    if (g_stop) {
      fprintf(stderr,
              "Server is stopping... after handling activated connections\n");
      fprintf(stderr, "# of activated connections: %d\n", active_connections);
    }
    int timeout = g_stop ? 100 : -1;
    int nfds = epoll_wait(epoll_fd, events, EVENT_NUM, timeout);
    if (nfds < 0) {
      if (errno == EINTR) {
        continue;
      }
    ...
    if (g_stop && active_connections == 0) {
      break;
    }
  }
